"""Add query_latency measurements to existing H3 run JSONs using small per-run sample sizes to be fast.

Usage:
  py -3 -u -m experiments.add_missing_query_latency_small_sample
"""
import json, os
...
for f in files:
    try:
        ...
        if not memmap_path or not Path(memmap_path).exists():
            ...
            continue
        n = cfg.get('n_docs')
        # determine sample_size smaller for big datasets
        if n >= 500000:
            sample_size = 10
        elif n >= 200000:
            sample_size = 20
        ...
        # measure latencies
        print(f"Measuring offline query latency using memmap {memmap_path} with sample_size={sample_size}")
        latencies = measure_offline_query_latency(memmap_path, docs_ids, q_embs, sample_size=sample_size, top_k=1)
        if not latencies:
        ...
        p90 = sorted_lat[int(0.90 * len(sorted_lat))]
